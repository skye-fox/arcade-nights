...
M.palette = require("arcade-nights.palette")
...
function M.setup(opts)
	...
	M.config = {
		plugins = {
			gitsigns = opts.plugins and opts.plugins.gitsigns == true,
			mini_icons = opts.plugins and opts.plugins.mini_icons == true,
			render_markdown = opts.plugins and opts.plugins.render_markdown == true,
			snacks = opts.plugins and opts.plugins.snacks == true,
		},
	}
	...
	vim.g.colors_name = "arcade-nights"

	local set = vim.api.nvim_set_hl
	local pal = M.palette

	if M.config.plugins.gitsigns then
		require("arcade-nights.plugins.gitsigns").setup(pal)
	end

	if M.config.plugins.mini_icons then
		require("arcade-nights.plugins.mini_icons").setup(pal)
	end

	if M.config.plugins.render_markdown then
		require("arcade-nights.plugins.render_markdown").setup(pal)
	end

	if M.config.plugins.snacks then
		require("arcade-nights.plugins.snacks").setup(pal)
	end
